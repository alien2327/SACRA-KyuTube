...
def make_csv(data):
    ...
    res.to_csv('./data_en_0.csv', index=False, encoding="cp932")
    return res

if __name__ == "__main__" :
    fp = open("./data_tag_en_1.csv", 'a', newline="", encoding="utf-8")
    writer = csv.writer(fp)
    data = []
    tag = ['個体','個体群','分子','細胞小器官','細胞','組織','器官','生物群集']
    tag_eng = ['Individual','Population','Molecule','Cell Organelle','Cell','Organization','Organ','Biological Community']
    tag_dic = dict(zip(tag, tag_eng))
    for tag_name in tag:
        base_url = f"https://kyutubebio.sci.kyoto-u.ac.jp/en/search?branch=All&title=&scientific_name=&name=&phylum=All&class=All&prefecture=All&country=&region=&target={tag_name}&keyword=&explanation=&paper=&photographer=All&copyright=All&flag=All&op=Search&page="
        resp = requests.get(base_url, verify = False)
        if resp.status_code == 200:
            soup = bs4.BeautifulSoup(resp.content, 'lxml')
            page_amount = int(soup.find("div", class_="view-header").text.strip().split()[1])
            page_amount /= 10
            for page in range(int(page_amount+1)):
                base_url = f"https://kyutubebio.sci.kyoto-u.ac.jp/en/search?branch=All&title=&scientific_name=&name=&phylum=All&class=All&prefecture=All&country=&region=&target={tag_name}&keyword=&explanation=&paper=&photographer=All&copyright=All&flag=All&op=Search&page=" + str(page)
                resp = requests.get(base_url, verify = False)
                if resp.status_code == 200:
                    soup = bs4.BeautifulSoup(resp.content, 'lxml')
                    cards = soup.find_all("div", class_="margin-bottom-lg")
                    for card in cards:
                        ...
                        print(tag_dic[tag_name], code, tag_name)
                        writer.writerow([tag_dic[tag_name], code_link])
                else:
                    print("Connection Failed")
        else:
            print("Connection Failed")